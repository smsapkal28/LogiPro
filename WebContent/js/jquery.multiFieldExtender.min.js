	...
	$.fn.EnableMultiField = function(options) {
		...
		return this
				.each(function() {
					...
					$(self).append(
							" <a href='#' class='" + options.linkClass + "'>"
									+ options.linkText + "</a>");
					$(self).find("." + options.linkClass).unbind().click(
							function() {
								return handleAdd($(this));
							});
					var myClone = $(self).clone();
					function handleAdd(elem) {
						var totalCount = parseInt($(self).attr(
								"TotalFieldsAdded"), 10);
						var fieldCount = parseInt($(self).attr("FieldCount"),
								10);
						if (options.maxItemsAllowedToAdd === null
								|| totalCount < options.maxItemsAllowedToAdd) {
							...
							if (options.enableRemove
									&& $(self).attr("uniqueId") != $(elem)
											.parent().attr("uniqueId")) {
								if ($(elem).parent().find(
										"." + options.removeLinkClass).length === 0) {
									$(elem).parent().append(
											" <a href='#' class='"
													+ options.removeLinkClass
													+ "'>"
													+ options.removeLinkText
													+ "</a>");
								}
								$(elem).parent().find(
										"." + options.removeLinkClass).unbind()
										.click(function() {
											return handleRemove($(this));
										});
							}
							$(newElem).attr("uniqueId",
									options.linkClass + Math.random());
							$(elem).parent().after(newElem);
							$(elem).parent().find("." + options.linkClass)
									.remove();
							$(newElem).find("." + options.linkClass).remove();
							$(newElem).find("." + options.removeLinkClass)
									.remove();
							if (options.enableRemove) {
								if ($(newElem).find(
										"." + options.removeLinkClass).length === 0) {
							...
							$(newElem).append(
									" <a href='#' class='" + options.linkClass
											+ "'>" + options.linkText + "</a>");
							newElem.find("." + options.linkClass).unbind()
									.click(function() {
										return handleAdd($(this));
									});
						...
						if (options.maxItemsAllowedToAdd !== null
							...
							if (options.maxItemReachedCallback !== null) {
								options
										.maxItemReachedCallback($(newElem),
												self);
							}
						}
						return false;
					...
					function handleRemove(elem) {
						...
						if (cnt) {
							...
							if ($(elem).parent().find("." + options.linkClass).length > 0) {
								...
								$(prevParent).append(
										" <a href='#' class='"
												+ options.linkClass + "'>"
												+ options.linkText + "</a>");
								prevParent.find("." + options.linkClass)
										.unbind().click(function() {
											return handleAdd($(this));
										});
							}
							if (options.maxItemsAllowedToAdd !== null
									&& totalCount < options.maxItemsAllowedToAdd) {
								$(self).siblings()
										.find("." + options.linkClass).show();
							}
							$(elem).parent().remove();
							if (options.removeEventCallback !== null) {
								options
										.removeEventCallback($(prevParent),
												self);
							}
						}
						return false;
