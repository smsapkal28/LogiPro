...
<div class="right_col" role="main">
	<div class="container">
			...
			<div class="title_left">
				<h3>
					<img src="${pageContext.request.contextPath}/Images/subject.png"
						width="35" height="35" />Library Collection
				</h3>
			</div>
		...
		<%
			...
			List<LibraryBookNotReturnEntity> libraryBookNotReturnedList = (List<LibraryBookNotReturnEntity>) request
					.getAttribute("libraryBookNotReturnedList");
									...
									<td>
										<table class="table table-bordered clickTable">
											<thead>
												<tr>
													<th colspan="5"
														style="text-align: center; background-color: #1ABB9C; color: #fff;"><%="Delay Return"%></th>
												</tr>
										...
										<table class="table table-bordered clickTable">
											<thead>
												<tr>
													<th colspan="5"
														style="text-align: center; background-color: #1ABB9C; color: #fff;"><%="Not Return"%></th>
												</tr>
											...
											<tbody class="collapse">
												...
												<%
													count2 = 0;
														total1 = 0;
														for (LibraryBookNotReturnEntity notReturn : libraryBookNotReturnedList) {
															if (library.getLib_Id() == notReturn.getLib_Id()) {
												%>
												<tr>
													<td><%=++count2%></td>
													<td><%
														for(LibraryStudentRegistrationEntity stud:libraryStudentList){
															if(notReturn.getStud_Id() == stud.getLib_Stud_Id())
																out.print(stud.getStud_Name());
														}
													...
													<td><%
														for(LibraryBookDetailEntity book:libraryBookList){
															if(book.getLib_Book_Id() == notReturn.getBook_Id())
																out.print(book.getBook_Name()+"/"+book.getBook_Edition()+"/"+book.getBook_Aurthor()+"/"+book.getBook_Publisher());
														}
													
													%></td>
													<td><%= notReturn.getFine_Date() %></td>
													<td><%=notReturn.getFine_Amount()%></td>

												</tr>
												<%
													total1 = total1 + notReturn.getFine_Amount();
																amount = amount + notReturn.getFine_Amount();
															}
														}
												%>
