...
<%
	...
	List<CustomerEntity> list_cust = (List<CustomerEntity>) request.getAttribute("custrecord");
...
function ajax_request()
{
	...
	var customername=document.getElementById('customer_ajx').value;
	var action="GetCustDetsils"
//alert("HIIIII=="+mblno);
$.ajax({
    type: "GET",
    url: "${pageContext.request.contextPath }/CustomerController",
    data: "customername="+customername+"&action="+action,
    dataType: "json",async:   false,
            success: function (jsondata) {
            	
            if (jsondata.response == 'SUCCESS') {
//          	alert("hii Resp");
            	$.each(jsondata.custdetails, function(i, object) {
                                    	var makeid = '#'+i;
                                        $(makeid).val(object);
                     					
                            });
            	if(jsondata.gstdetails != null)
                {
                        $.each(jsondata.gstdetails, function(i, object) {
                                var array = new Array();
                                $.each(object, function(property, value) {
                                        array[property] = object[property];
                        });
                        AddGST(array[0],array[1],array[2],array[3],array[4],array[5],array[6],array[7],array[8],array[9],array[10]);
                        });
                }
            	if(jsondata.alertdetails != null)
                {
                        $.each(jsondata.alertdetails, function(i, object) {
                                var array = new Array();
                                $.each(object, function(property, value) {
                                        array[property] = object[property];
                        });
                        AddAlert(array[0],array[1],array[2],array[3]);
                        });
                }
            	if(jsondata.creditdetails != null)
                {
                        $.each(jsondata.creditdetails, function(i, object) {
                                var array = new Array();
                                $.each(object, function(property, value) {
                                        array[property] = object[property];
                        });
                                AddCreditRate(array[0],array[1],array[2],array[3]);
                        });
                }  
...
function AddAlert(alertname,shiptype,toname,toemaiid)
{
  ...
  {
        el.options[0] = new Option(alertname,alertname);
  }
  <%int k = 0;
			...
			<div class="widget_wrap">
				...
				<div class="widget_content">
					<form
						...
						<input type='hidden' name="action" value="CustomerModify" /> <input
							...
							<div class="form_grid_12 multiline">
								<div class="form_input">
									<div class="form_grid_2 alpha">
										...
										<br>
										<div class="form_input">
											<select style="width: 150px;" class="chzn-select-deselect" id="customer_ajx"
												tabindex="16">
												<option value="" selected>Select</option>
												<%
											for (CustomerEntity list : list_cust) {
										%>
										<option value="<%=list.getCustomername()%>"><%=list.getCustomername()%></option>
										<%
											}
										%>												
											</select>
