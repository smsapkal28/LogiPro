...
<script>
	function AddCharges1(containerno, containertype, customsealno, agentsealno) {
		...
		el.onclick = function() {
			deleterw(this.parentNode.parentNode.rowIndex)
		};
...
<script>
	function deleterw(i) {
		...
		for (var j = i + 1; j <= rows3; j++, i++) {
			...
			document.getElementById("containertype" + j).id = "containertype"
					+ i;
	...
	function Validate() {
		...
		for (var i = 1; i < rowcontainer; i++) {
			if (document.getElementById('containerno' + i).value == "") {
				...
				document.forms[0].containerlist.value += document
						.getElementById('containerno' + i).value
						+ "*";
			}
			if (document.getElementById('containertype' + i).value == "") {
				...
				document.forms[0].containerlist.value += document
						.getElementById('containertype' + i).value
						+ "*";
			}
			if (document.getElementById('customsealno' + i).value == "") {
				...
				document.forms[0].containerlist.value += document
						.getElementById('customsealno' + i).value
						+ "*";
			}
			if (document.getElementById('agentsealno' + i).value == "SELECT") {
				...
				document.forms[0].containerlist.value += document
						.getElementById('agentsealno' + i).value
						+ "*";
			}
		}
...
<script>
	$(document).ready(function() {
		...
		$("#mbldate").datepicker({  dateFormat: 'dd/mm/yy'  });
		$("#mblissuedate").datepicker({  dateFormat: 'dd/mm/yy'  });
...
function ajax_request()
{
	...
	var mblno=document.getElementById('mblno_ajx').value;
//alert("HIIIII=="+mblno);
$.ajax({
    type: "GET",
    url: "${pageContext.request.contextPath }/MasterBLModifyController",
    data: "sbillno1="+mblno,
...
//          	alert("hii Resp");
            	$.each(jsondata.mbldetails, function(i, object) {
                                    	var makeid = '#'+i;
                                        $(makeid).val(object);
                     					
                            });
            	if(jsondata.contdetails != null)
                {
                        $.each(jsondata.contdetails, function(i, object) {
                                var array = new Array();
                                $.each(object, function(property, value) {
                                        array[property] = object[property];
                        });
                        AddCharges1(array[0],array[1],array[2],array[3]);
                        });
                }
	...
	<div class="grid_container">
		<div class="grid_12 full_block">
			<div class="widget_wrap">
				...
				<%
				List<MasterBLEntity> mbl_list = (List<MasterBLEntity>) request.getAttribute("mblrecord");
				System.out.println("=========mbl_list======="+mbl_list);
				%>
				<div class="widget_content">
					<form
						action="${pageContext.request.contextPath }/MasterBLController"
						method="post" class="form_container">
						<input type='hidden' name="action" value="MasterBLEntryModify" /> 
						<input type='hidden' name="containerlist" />
						<input type='hidden' name="mblno_id" id="mblno_id" />						
						<div id="loginForm" style="display: none">
							<div class="form_grid_12 multiline">
								<div class="form_input">
									<div class="form_grid_2 alpha">
										<label class="field_title">MBL No. <span class="req">*</span></label>
										<div class="form_input">
											<select style="width: 150px;" class="chzn-select-deselect" id="mblno_ajx"
												tabindex="16">
												<option value="" selected>Select</option>
												<%
											for (MasterBLEntity list : mbl_list) {
										%>
										<option value="<%=list.getMblno()%>"><%=list.getMblno()%></option>
										<%
											}
										%>												
											</select>
									...
									<div class="submit" align="center">
										<input type="submit" onclick="return ajax_request();"
											value="Next" />
									</div>
										...
										<thead>
											<tr>
												...
												<th><a class='toolbar' href='#'
													onClick="AddCharges1('','','',''); return false;"><img
														src='../images/Add.png' alt='Add Rows' align='middle'
														name='lookup' border='0'></a></th>
											</tr>
										</thead>
									</table>
								</fieldset>
								...
								<div class="form_grid_12" style="text-align: center;">
									<div class="form_input">
										<button type="submit" class="btn_small btn_blue" name="Master"
											value="MasterBLEntry" onClick="return Validate();">
											<span>Submit</span>
										</button>
