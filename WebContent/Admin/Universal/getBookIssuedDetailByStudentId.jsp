	...
	<tbody>
		...
		<tr>
				...
				<%
					for (LibrarySettingEntity setting : librarySettingList) {
								if (setting.getLib_Id() == libraryBook.getLib_Id()){
									out.print((delay / setting.getDelay_Return_Time()) * setting.getDelay_Return_Fine());
									delay_Time = setting.getDelay_Return_Time();

									fine = ((delay / setting.getDelay_Return_Time()) * setting.getDelay_Return_Fine());
							}}
				%>
			</td>
			<td><form class="form-horizontal form-label-left input_mask"
					...
					method="post">
					<input type='hidden' name="action" value="AddReturnDelayDetail"
						class="form-control" />
						...
						class="form-control" />
					<button type="submit" id="return_Btn" class="btn btn-info">
